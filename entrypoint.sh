...
then
  ...
  then
    ...
    roslaunch --wait --local -v /robot_ws/src/multirobot_sim/launch/uav-docker.launch
  elif [ "$ROBOT_TYPE" == "ugv" ]
  then
    echo "robot type is ugv"
    roslaunch --wait --local -v /robot_ws/src/multirobot_sim/launch/ugv-docker.launch
  else
